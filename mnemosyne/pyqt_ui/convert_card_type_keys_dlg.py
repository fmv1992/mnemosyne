...
from mnemosyne.pyqt_ui.ui_convert_card_type_keys_dlg import Ui_ConvertCardTypeKeysDlg
    ...
    def combobox_updated(self):
        ...
        for old_fact_key, old_fact_key_name in self.old_card_type.fact_keys_and_names:
            ...
            if new_fact_key_name != _("<none>"):
                ...
                new_fact_key = self.new_card_type.fact_key_with_name(new_fact_key_name)
        ...
        if self.check_required_fact_keys:
            for fact_key in self.new_card_type.required_fact_keys:
                if fact_key not in self.correspondence.values():
                    self.ok_button.setEnabled(False)
                    if len(self.correspondence) == len(self.old_card_type.fact_keys()):
                        QtWidgets.QMessageBox.critical(
                            self, _("Mnemosyne"), _("A required field is missing.")
                        )
                    return
