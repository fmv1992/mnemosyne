...
class ManagePluginsDlg(
    ...
    def accept(self):
        ...
        for index in range(self.plugin_list.count()):
            ...
            if (
                list_item.checkState() == QtCore.Qt.CheckState.Checked
                and self.previously_active[plugin_name] == False
            ):
                self.plugin_with_name[plugin_name].activate()
            elif (
                list_item.checkState() == QtCore.Qt.CheckState.Unchecked
                and self.previously_active[plugin_name] == True
            ):
    ...
    def delete_plugin(self):
        ...
        question = (
            _("Are you sure you want to delete the plugin")
            + ' "'
            + plugin_name
            + '" '
            + _("and not just deactivate it?")
        )
        answer = self.main_widget().show_question(
            question, _("Delete"), _("Cancel"), ""
        )
        if answer == 1:  # Cancel
            return
