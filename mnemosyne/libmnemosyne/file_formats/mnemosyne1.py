...
class Mnemosyne1(MediaPreprocessor):
    ...
    def create_card_from_item(self, item, extra_tag_names):
        ...
        # Map.
        if (
            item.id + ".inv" in self.items_by_id
            and "answerbox: overlay" in item.q
            and "<img " in item.q
        ):
        ...
        # Front-to-back.
            ...
            item.id + ".inv" not in self.items_by_id
            and item.id + ".tr.1" not in self.items_by_id
        ):
            card_type = self.card_type_with_id("1")
            fact_data = {"f": item.q, "b": item.a}
            if not fact_data["f"]:
    ...
    def set_card_attributes(self, card, item):
        ...
        card.last_rep = self.scheduler().midnight_UTC(
            self.starttime + item.last_rep * DAY
        )
        card.next_rep = self.scheduler().midnight_UTC(
            self.starttime + item.next_rep * DAY
        )
