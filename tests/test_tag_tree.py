...
class TestTagTree(MnemosyneTest):
    ...
    def test_2(self):
        ...
        card_type = self.card_type_with_id("1")
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["tag_1"]
        )[0]
        self.controller().save_file()
        from mnemosyne.libmnemosyne.tag_tree import TagTree
    ...
    def test_3(self):
        ...
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["Z"]
        )[0]
        ...
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["a::c"]
        )[0]
        fact_data = {"f": "question5", "b": "answer5"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["b::c::d"]
        )[0]
        from mnemosyne.libmnemosyne.tag_tree import TagTree
    ...
    def test_rename_to_existing_tag(self):
        ...
        fact_data = {"f": "question4", "b": "answer4"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["tag1"]
        )[0]
        fact_data = {"f": "question", "b": "answer"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["tag2"]
        )[0]
        from mnemosyne.libmnemosyne.tag_tree import TagTree
    ...
    def test_rename_to_existing_tag_2(self):
        ...
        card = self.controller().create_new_cards(
            ...
            card_type,
            grade=-1,
            tag_names=["Xx::vb::test", "Xx::aa::vb::test"],
        )[0]
        from mnemosyne.libmnemosyne.tag_tree import TagTree
        ...
        assert (
            ...
            .con.execute("select tags from cards where _id=?", (card._id,))
            .fetchone()[0]
        )
        assert (
            self.database()
    ...
    def test_rename_to_empty_2(self):
        ...
        fact_data = {"f": "question4", "b": "answer4"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["A::tag1"]
        )[0]
        from mnemosyne.libmnemosyne.tag_tree import TagTree
    ...
    def test_delete_2(self):
        ...
        fact_data = {"f": "question5", "b": "answer5"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["b::c::d", "b"]
        )[0]
        from mnemosyne.libmnemosyne.tag_tree import TagTree
    ...
    def test_count_1(self):
        ...
        fact_data = {"f": "question4", "b": "answer4"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["b"]
        )[0]
        fact_data = {"f": "question", "b": "answer"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["b", "b"]
        )[0]
    ...
    def test_count_2(self):
        ...
        fact_data = {"f": "question4", "b": "answer4"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["X::a"]
        )[0]
        fact_data = {"f": "question", "b": "answer"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["X::a", "X::b"]
        )[0]
