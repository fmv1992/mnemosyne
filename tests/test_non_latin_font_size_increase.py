...
class TestNonLatinFontSizeIncrease(MnemosyneTest):
    def test_1(self):
        ...
        assert (
            """<font style=\"font-size:14pt\">""" + chr(40960) + "</font>"
            in card.question()
        )

    def test_2(self):
        fact_data = {
            "f": "<protect>question " + chr(40960) + "</protect>",
            "b": "answer",
        }
        card_type = self.card_type_with_id("1")
        card = self.controller().create_new_cards(
    ...
    def test_3(self):
        fact_data = {"f": "question " + chr(40960), "p_1": "", "m_1": "answer"}
    ...
    def test_4(self):
        fact_data = {"f": "question " + chr(40960), "m_1": "answer"}
        card_type = self.card_type_with_id("3")
    ...
    def test_5(self):
        fact_data = {"f": "question " + chr(40960) + "tail", "b": "answer"}
        card_type = self.card_type_with_id("1")
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=[]
        )[0]
