...
class TestDatabase(MnemosyneTest):
    def setup_method(self):
        ...
        self.mnemosyne.components.append(("test_database", "Widget"))
    ...
    def test_new_cards(self):
        ...
        old_card = self.controller().create_new_cards(
        ...
        list(self.database().cards_from_fact(fact))[0]
    ...
    def test_schedule_on_same_day(self):
        ...
        assert (
            self.database().sister_card_count_scheduled_between(
                card_2, card_1.next_rep, card_1.next_rep + DAY
            )
            == 1
        )
        assert (
            self.database().sister_card_count_scheduled_between(
                card_3, card_1.next_rep, card_1.next_rep + DAY
            )
            == 0
        )
        assert (
            self.database().sister_card_count_scheduled_between(
                card_1, card_1.next_rep, card_1.next_rep + DAY
            )
            == 0
        )
    ...
    def test_purge_backups(self):
        ...
        for count in range(15):
            open(os.path.join(backup_dir, "default-%d.db" % count), "w")
        self.mnemosyne.finalise()
    ...
    def test_link_inverse_cards(self):
        ...
        card_1 = self.controller().create_new_cards(
            fact_data, card_type_1, grade=-1, tag_names=["tag_1"]
        )[0]
    ...
    def test_link_inverse_cards_2(self):
        ...
        fact_data = {"f": "answer", "b": "question"}
        card_2 = self.controller().create_new_cards(
            fact_data, card_type_1, grade=-1, tag_names=["tag_2"]
        )[0]
    ...
    def test_link_inverse_cards_3(self):
        ...
        card_type_2 = self.card_type_with_id("2")
        card_1 = self.controller().create_new_cards(
            fact_data, card_type_2, grade=-1, tag_names=["tag_1"]
        )[0]

        fact_data = {"f": "question", "b": "answer"}
        card_2 = self.controller().create_new_cards(
            fact_data, card_type_2, grade=-1, tag_names=["tag_2"]
        )[0]
    ...
    def test_add_tag_to_card(self):
        ...
        sql_res = (
            self.database()
            .con.execute("select event_type, object_id from log where _id=10")
            .fetchone()
        )
        ...
        assert (
            self.database()
            .con.execute("select count() from tags_for_card")
            .fetchone()[0]
            == 2
        )
        assert len(new_card.tags) == 2
    ...
    def test_remove_tag_from_card(self):
        ...
        card_type = self.card_type_with_id("1")
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["a", "b"]
        )[0]
        assert (
            self.database()
            .con.execute(
        ...
        sql_res = (
            self.database()
            .con.execute("select event_type, object_id from log where _id=12")
            .fetchone()
        )
        assert sql_res[0] == EventTypes.EDITED_CARD
        assert sql_res[1] == card.id
    ...
    def test_tags_for_cards(self):
        ...
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["c"]
        )[0]
        card__id_3 = card._id
    ...
    def test_upgrade_2(self):
        shutil.copy(
            os.path.join("tests", "files", "non_unique_card_ids.db"),
    ...
    def test_known_recognition(self):
        ...
        assert set(
            (
                self.database().known_recognition_questions_from_card_types_ids(
                    ["6", "3::my_3", "3::my_3_bis"]
                )
            )
        ) == set(["yes_1", "yes_2"])
    ...
    def _create_n_test_cards(self, n):
        ...
        for i in range(n):
            ...
            c = self.controller().create_new_cards(
                fact_data, card_type, grade=-1, tag_names=["default"]
            )
            cards.append(c[0])
        return cards
