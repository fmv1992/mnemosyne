...
class SM2Controller(ReviewController):
    ...
    tooltip = [["", "", "", "", "", "", ""], ["", "", "", "", "", "", ""]]
    tooltip[ACQ_PHASE][0] = _("You don't remember this card yet.")
    tooltip[ACQ_PHASE][1] = _(
        "Like '0', but it's getting more familiar. Show it less often."
    )
    tooltip[ACQ_PHASE][2] = _(
        "You've memorised this card now, and might remember it tomorrow."
    )
    tooltip[ACQ_PHASE][3] = _(
        "You've memorised this card now, and should remember it tomorrow."
    )
    tooltip[ACQ_PHASE][4] = _(
        "You've memorised this card now, and might remember it in 2 days."
    )
    tooltip[ACQ_PHASE][5] = _(
        "You've memorised this card now, and should remember it in 2 days."
    )
    ...
    tooltip[RET_PHASE][4] = _(
        "Correct answer, with some effort. The interval was probably just right."
    )
    tooltip[RET_PHASE][5] = _(
        "Correct answer, but without any difficulties. The interval was probably too short."
    )
    ...
    def grade_answer(self, grade):
        ...
        if self.config()["show_intervals"] == "status_bar":
            ...
            self.main_widget().set_status_bar_message(
    ...
    def update_qa_area(self, redraw_all=False):
        ...
        if self.card is not None:
            if (
                ...
                or self.card.fact_view.a_on_top_of_q
            ):
    ...
    def update_grades_area(self):
        ...
        if (
            self._state == "SELECT GRADE"
            and self.config()["show_intervals"] == "buttons"
        ):
        ...
        for grade in range(0, 6):
            ...
            if (
                self._state == "SELECT GRADE"
                and self.config()["show_intervals"] == "tooltips"
            ):
                import math

                interval = self.scheduler().process_answer(
