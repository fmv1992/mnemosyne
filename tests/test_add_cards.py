...
class TestAddCards(MnemosyneTest):
    def setup_method(self):
        ...
        self.mnemosyne.components.append(("test_add_cards", "Widget"))
    ...
    def test_delete(self):
        ...
        assert set(tag.name for tag in card_1.tags) == set(
            tag.name for tag in card_2.tags
        )
    ...
    def test_change_tag(self):
        ...
        self.controller().edit_card_and_sisters(
            card, fact_data, card_type, ["new"], correspondence={}
        )
    ...
    def test_edit_untagged(self):
        ...
        card_type = self.card_type_with_id("1")
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["tag"]
        )[0]
        assert self.database().fact_count() == 1
        assert self.database().card_count() == 1
        ...
        self.controller().edit_card_and_sisters(
            new_card, new_card.fact.data, new_card.card_type, [" "], []
        )
    ...
    def test_edit_untagged_2(self):
        ...
        self.controller().edit_card_and_sisters(
            new_card, new_card.fact.data, new_card.card_type, ["tag"], []
        )
    ...
    def test_duplicate_2(self):
        ...
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["b", "c"]
        )[0]
    ...
    def test_log(self):
        fact_data = {"f": "test question", "b": "test answer"}
        card_type = self.card_type_with_id("1")
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=3, tag_names=["test tag name"]
        )[0]
        self.controller().save_file()
        ...
        sql_res = (
            self.database()
            .con.execute("select event_type, object_id from log where _id=7")
            .fetchone()
        )
        ...
        sql_res = (
            self.database()
            .con.execute("select event_type from log where _id=8")
            .fetchone()
        )
    ...
    def test_different_tags_per_sister_card(self):
        ...
        self.controller().edit_card_and_sisters(
            card, card.fact.data, card.card_type, ["extra2"], []
        )
        card = self.database().card(card._id, is_id_internal=True)
        card_2 = self.database().card(card_2._id, is_id_internal=True)
    ...
    def test_optional_keys(self):
        ...
        assert (
            self.database()
            .con.execute("select count() from data_for_fact where key='n'")
            .fetchone()[0]
            == 0
        )
