...
class CardTypeDelegate(TagDelegate):
    ...
    def setEditorData(self, editor, index):
        ...
        self.previous_node_name = index.model().data(index).rsplit(" (", 1)[0]
...
class CardTypesTreeWdgt(TagsTreeWdgt):
    ...
    def display(self, criterion=None):
        ...
        for card_type in self.database().sorted_card_types():
            ...
            if count_for_card_type[
                card_type
            ] == 0 and self.database().is_user_card_type(card_type):
            ...
            if self.database().is_user_card_type(card_type):
                ...
                card_type_item.setFlags(
                    card_type_item.flags() | QtCore.Qt.ItemFlag.ItemIsEditable
                )
            for fact_view in card_type.fact_views:
    ...
    def restore_criterion(self):
        ...
        old_card_type_ids = set(
            [
                cursor[0]
                for cursor in self.saved_criterion.deactivated_card_type_fact_view_ids
            ]
        )
        ...
        for card_type_id, fact_view_id in deactivated_card_type_fact_view_ids:
            if card_type_id in deleted_card_type_ids:
                self.saved_criterion.deactivated_card_type_fact_view_ids.discard(
