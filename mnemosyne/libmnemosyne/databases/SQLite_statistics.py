...
class SQLiteStatistics(object):
    ...
    def scheduled_count(self, timestamp):
        count = self.con.execute(
            ...
            and ret_reps_since_lapse<=?""",
            (timestamp, self.config()["max_ret_reps_since_lapse"]),
        ).fetchone()[0]
        return count
    ...
    def easinesses_for_tag(self, tag, active_only):
        ...
        return [cursor[0] for cursor in self.con.execute(query, (tag._id,))]
    ...
    def card_count_scheduled_between(self, start, stop):
        return self.con.execute(
            ...
            (start, stop, self.config()["max_ret_reps_since_lapse"]),
        ).fetchone()[0]
    ...
    def card_count_scheduled_n_days_ago(self, n):
        ...
        for cursor in self.con.execute(
            """select acq_reps, object_id from log
