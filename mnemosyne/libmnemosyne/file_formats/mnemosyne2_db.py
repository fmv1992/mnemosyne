...
class Mnemosyne2Db(FileFormat):
    ...
    def do_import(self, filename, extra_tag_names=""):
        ...
        receiving_database_filename = expand_path(
            self.config()["last_database"], data_dir
        )
        ...
        if len(tag_ids_to_import.intersection(current_tag_ids)) >= 2:
            answer = self.main_widget().show_question(
                _(
                    "It looks like you've imported this database before. Importing it twice will generate duplicate log entries, which will skew your statistics. Do you want to continue?"
                ),
                _("Abort"),
                _("Continue"),
                "",
        ...
        user_card_types = [
            card_type
            for card_type in db.card_types_in_use()
