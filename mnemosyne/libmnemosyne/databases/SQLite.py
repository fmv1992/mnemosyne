...
class SQLite(
    ...
    def new(self, path):
        ...
        if self.store_pregenerated_data:
            ...
            self.con.executescript(SCHEMA.substitute(pregenerated_data=""))
        self.con.execute(
            "insert into global_variables(key, value) values(?,?)",
            ("version", self.version),
        )
        self.con.execute(
            """insert into partnerships(partner, _last_log_id)
            values(?,?)""",
            ("log.txt", 0),
        )
        self.config()["last_database"] = contract_path(
            self._path, self.config().data_dir
        )
    ...
    def load(self, path):
        ...
        try:
            sql_res = self.con.execute(
                """select value from global_variables
                where key=?""",
                ("version",),
            ).fetchone()
        except:
    ...
    def is_empty(self):
        return (
            ...
            and self.con.execute(
                """select count() from log where event_type=? or
            event_type=? or event_type=? or event_type=?""",
                (
                    EventTypes.ADDED_TAG,
                    EventTypes.ADDED_FACT,
    ...
    def get_or_create_tag_with_name(self, name):
        ...
        if name.startswith("::"):
            name = name[2:]
        sql_res = self.con.execute(
            """select _id, id, name, extra_data from
            tags where name=?""",
            (name,),
        ).fetchone()
        if sql_res:
    ...
    def add_tag(self, tag):
        ...
        self.con.execute(
            """insert into tags(name, extra_data, id)
            values(?,?,?)""",
            (tag.name, self._repr_extra_data(tag.extra_data), tag.id),
        )
        ...
        if not saved_criterion:
            ...
            for node in tag.name.split("::"):
                ...
                if (
                    partial_tag_name != tag.name
                    and partial_tag_name in existing_tag_for_name
                ):
    ...
    def update_tag(self, tag):
        ...
        stored_name = self.con.execute(
            "select name from tags where _id=?", (tag._id,)
        ).fetchone()[0]
        if (
            new_name != stored_name
            and self.con.execute(
                """select 1 from
            tags where name=? limit 1""",
                (new_name,),
            ).fetchone()
            is not None
        ):
            _existing_tag_id = self.con.execute(
                """select _id from tags where
            name=?""",
                (new_name,),
            ).fetchone()[0]
            _card_ids_affected = [
            ...
            for _card_id in _card_ids_affected:
                ...
                if (
                    self.con.execute(
                        """select 1 from tags_for_card where
                    _tag_id=? and _card_id=? limit 1""",
                        (_existing_tag_id, _card_id),
                    ).fetchone()
                    is not None
                ):
                    self.con.execute(
                        """delete from tags_for_card where
                    _tag_id=? and _card_id=?""",
                        (tag._id, _card_id),
                    )
                # If not, update the link.
                else:
                    self.con.execute(
                        """update tags_for_card set _tag_id=?
                        where _tag_id=?""",
                        (_existing_tag_id, tag._id),
                    )
        ...
        # Regular case.
        self.con.execute(
            """update tags set name=?, extra_data=? where
            _id=?""",
        ...
        for _card_id in _card_ids:
            ...
            for cursor in self.con.execute(
                ...
                tag_name = self.con.execute(
                    "select name from tags where _id=?", (cursor[0],)
                ).fetchone()[0]
            ...
            self.con.execute(
                "update cards set tags=? where _id=?", (tag_string, _card_id)
            )
    ...
    def tags(self):
        ...
        result = [
            self.tag(cursor[0], is_id_internal=True)
            for cursor in self.con.execute("select _id from tags")
        ]
            ...
            """,
                {
                    ...
                    "event_type": EventTypes.REPETITION,
                },
            ).fetchall()
        )
    ...
    def fact_ids_newly_learned_today(self, start_of_day, end_of_day):
        return (
            ...
            and log.event_type=? and log.grade>=2 and log.ret_reps==0""",
                (start_of_day, end_of_day, EventTypes.REPETITION),
            ).fetchall()
        )
    ...
    def add_card(self, card):
        ...
        self.con.execute(
            """insert into cards(id, card_type_id,
    ...
    def card(self, id, is_id_internal):
        ...
        for cursor in self.con.execute(
            """select _tag_id from tags_for_card
            where _card_id=?""",
            (card._id,),
        ):
    ...
    def update_card(self, card, repetition_only=False):
        ...
        if not repetition_only:
            self.current_criterion().apply_to_card(card)
        self.con.execute(
            """update cards set grade=?, next_rep=?, last_rep=?,
        ...
        if repetition_only:
            return
        self.con.execute(
            """update cards set card_type_id=?, _fact_id=?,
    ...
    def add_tag_to_cards_with_internal_ids(self, tag, _card_ids):
        ...
        self.con.executemany(
            """delete from tags_for_card where _tag_id=?
            and _card_id=?""",
            arguments,
        )
        ...
        arguments = ((tag._id, _card_id) for _card_id in _card_ids)
        self.con.executemany(
            """insert into tags_for_card(_tag_id, _card_id)
            values(?,?)""",
            arguments,
        )
        if self.store_pregenerated_data:
        ...
        for _card_id in _card_ids:
            card_id = self.con.execute(
                "select id from cards where _id=?", (_card_id,)
            ).fetchone()[0]
            self.con.execute(
    ...
    def remove_tag_from_cards_with_internal_ids(self, tag, _card_ids):
        ...
        arguments = (
            (_tag_id_untagged, _card_id)
            for _card_id in set(_card_ids).difference(_card_ids_tagged)
        )
        self.con.executemany(
            """insert into tags_for_card(_tag_id, _card_id)
            values(?,?)""",
    ...
    def add_fact_view(self, fact_view):
        self.con.execute(
            """insert into fact_views(id, name, q_fact_keys,
    ...
    def update_fact_view(self, fact_view):
        self.con.execute(
            """update fact_views set name=?, q_fact_keys=?,
            a_fact_keys=?, q_fact_key_decorators=?, a_fact_key_decorators=?,
    ...
    def delete_fact_view(self, fact_view):
        self.con.execute("delete from fact_views where id=?", (fact_view.id,))
    ...
    def has_fact_view_with_id(self, id):
        return (
            self.con.execute(
                "select 1 from fact_views where id=? limit 1", (id,)
            ).fetchone()
            is not None
        )
    ...
    def add_card_type(self, card_type):
        ...
        if not saved_criterion:
            ...
            answer = self.main_widget().show_question(
                ...
                % (card_type.name, saved_criterion.name),
                _("Yes"),
                _("No"),
                "",
            )
    ...
    def card_type(self, id, is_id_internal):
        ...
        card_type.fact_views = [
            ...
            for fact_view_id in eval(sql_res[4])
        ]
        return card_type
    ...
    def is_user_card_type(self, card_type):
        return (
            self.con.execute(
                "select 1 from card_types where id=? limit 1", (card_type.id,)
            ).fetchone()
            is not None
        )
    ...
    def delete_card_type(self, card_type):
        ...
        # so as not to upset the sync protocol.
        self.con.execute("delete from card_types where id=?", (card_type.id,))
        self.component_manager.unregister(card_type)
    ...
    def add_criterion(self, criterion):
        self.con.execute(
            """insert into criteria (id, name, type, data)
            values(?,?,?,?)""",
            (
                criterion.id,
                criterion.name,
    ...
    def criterion(self, id, is_id_internal):
        if is_id_internal:
            sql_res = self.con.execute(
                """select _id, id, name, type, data
                from criteria where _id=?""",
                (id,),
            ).fetchone()
        else:
            sql_res = self.con.execute(
        ...
        for criterion_class in self.component_manager.all("criterion"):
            if criterion_class.criterion_type == sql_res[3]:
                criterion = criterion_class(self.component_manager, sql_res[1])
    ...
    def update_criterion(self, criterion):
        self.con.execute(
            """update criteria set name=?, type=?, data=?
            where id=?""",
            (
                criterion.name,
                criterion.criterion_type,
    ...
    def criteria(self):
        return (
            self.criterion(cursor[0], is_id_internal=True)
    ...
    def has_criterion_with_id(self, id):
        return (
            self.con.execute(
                "select 1 from criteria where id=? limit 1", (id,)
            ).fetchone()
            is not None
        )
    ...
    def cards_from_fact(self, fact):
        return list(
            self.card(cursor[0], is_id_internal=True)
            for cursor in self.con.execute(
    ...
    def duplicates_for_fact(self, fact, card_type):
        ...
        for fact_key in card_type.unique_fact_keys:
            if fact._id:
                for cursor in self.con.execute(
                    """select _fact_id from
                    data_for_fact where key=? and value=? and not
                    _fact_id=?""",
                    (fact_key, fact[fact_key], fact._id),
                ):
        ...
        for _fact_id in _fact_ids:
            this_card_type_id = self.con.execute(
                """select card_type_id
                from cards where _fact_id=?""",
                (_fact_id,),
            ).fetchone()[0]
    ...
    def link_inverse_cards(self):
        ...
        for key in set(_fact_id_for_front.keys()).intersection(
            list(_fact_id_for_back.keys())
        ):
            ...
            if (
                _fact_id_1 not in _card_id_for__fact_id
                or _fact_id_2 not in _card_id_for__fact_id
            ):
                continue
            ...
            # 'fact_id_for_back'.
            if (
                "b" in card_1.fact.data
                and "b" in card_2.fact.data
                and (
                    card_1.fact["f"] != card_2.fact["b"]
                    or card_1.fact["b"] != card_2.fact["f"]
                )
            ):
                continue
    ...
    def cards_due_for_ret_rep(self, timestamp, sort_key="", limit=-1):
        ...
        return (
            ...
            and ?>=next_rep and ret_reps_since_lapse<=? order by %s limit ?"""
                % sort_key,
                (timestamp, self.config()["max_ret_reps_since_lapse"], limit),
            )
        )
    ...
    def recently_memorised_count(self, max_ret_reps):
        return self.con.execute(
            """select count() from cards where active=1
    ...
    def scheduler_data_count(self, scheduler_data, max_ret_reps=-1):
        ...
        return self.con.execute(
            ...
            where active=1 and scheduler_data=? %s """
            % extra_cond,
            (scheduler_data,),
        ).fetchone()[0]
    ...
    def has_already_warned_today(self, start_of_day, end_of_day):
        result = (
            ...
            log.timestamp <? and log.event_type=?""",
                (start_of_day, end_of_day, EventTypes.WARNED_TOO_MANY_CARDS),
            )
            .fetchall()
        )
