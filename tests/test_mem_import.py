...
class TestMemImport(MnemosyneTest):
    ...
    @mock.patch(
        ...
        mock.Mock(side_effect=[FileNotFoundError, IndexError("Mocked Error")]),
    )
    ...
    def test_media_quotes(self):
        filename = os.path.join(
            os.getcwd(), "tests", "files", "basedir_media", "default.mem"
        )
        self.mem_importer().do_import(filename)
        assert (
            self.database()
    ...
    def test_sound(self):
        os.mkdir(os.path.join(os.getcwd(), "tests", "files", "soundfiles"))
        soundname = os.path.join(
            os.path.join(os.getcwd(), "tests", "files", "soundfiles", "a.ogg")
        )
        open(soundname, "w")
    ...
    def test_dups(self):
        ...
        self.review_controller().reset()
        assert (
            self.review_controller().card.fact["loc"]
            == """<b>Freistaat Th\xfcringen (Free State of Thuringia)</b>"""
        )
        assert (
            self.review_controller().card.tag_string()
    ...
    def test_two_mem_files_sharing_same_logs(self):
        ...
        filename = os.path.join(
            os.getcwd(), "tests", "files", "basedir_2_mem", "deck2.mem"
        )
        self.mem_importer().do_import(filename)
        assert (
            self.database()
