...
class CardModel(QtSql.QSqlTableModel, Component):
    ...
    def __init__(self, **kwds):
        ...
        for card_type_id, rgb in self.config()["background_colour"].items():
    ...
    def data(self, index, role=QtCore.Qt.ItemDataRole.DisplayRole):
        if role == QtCore.Qt.ItemDataRole.ForegroundRole:
            ...
            card_type_id = QtSql.QSqlTableModel.data(self, card_type_id_index)
        ...
        if (
            ...
            and column not in (QUESTION, ANSWER, TAGS)
        ):
...
class QA_Delegate(QtWidgets.QStyledItemDelegate, Component):
    ...
    def tag(self, _id):
        query = QtSql.QSqlQuery("select name from tags where _id=%d" % (_id,))
        query.first()
    ...
    def card(self, _id):
        query = QtSql.QSqlQuery(
            """select _fact_id, card_type_id,
            fact_view_id, extra_data from cards where _id=%d"""
            % (_id,)
        )
        query.first()
    ...
    def paint(self, painter, option, index):
        ...
        rect = style.subElementRect(
            QtWidgets.QStyle.SubElement.SE_ItemViewItemText, option, None
        )
        # Render.
        painter.save()
    ...
    def paint_webengine(self, painter, option, index):
        ...
        if option.state & QtWidgets.QStyle.StateFlag.State_Selected:
            force_text_colour = option.palette.color(
                QtWidgets.QPalette.ColorGroup.Active,
                QtWidgets.QPalette.ColorRole.HighlightedText,
            ).rgb()
        else:
        ...
        # Background colour.
        rect = style.subElementRect(
            QtWidgets.QStyle.SubElement.SE_ItemViewItemText, option
        )
        if option.state & QtWidgets.QStyle.StateFlag.State_Selected:
            background_colour = option.palette.color(
                QtWidgets.QPalette.ColorGroup.Active,
...
class BrowseCardsDlg(
    ...
    tip_after_n_times = {
        ...
        18: _(
            "You change the relative size of the card list, card type tree and tag tree by dragging the dividers between them."
        ),
        21: _(
    ...
    def __init__(self, **kwds):
        ...
        self.label_1 = QtWidgets.QLabel(
            _("Show cards from these card types:"), self.container_1
        )
        self.layout_1.addWidget(self.label_1)
        self.card_type_tree_wdgt = CardTypesTreeWdgt(
            acquire_database=self.unload_qt_database,
            component_manager=kwds["component_manager"],
            parent=self.container_1,
        )
        self.card_type_tree_wdgt.card_types_changed_signal.connect(
    ...
    def menu_edit(self, index=None):
        ...
        self.edit_dlg = self.component_manager.current("edit_card_dialog")(
            card,
            allow_cancel=True,
            started_from_card_browser=True,
            parent=self,
    ...
    def page_up_down_edit(self, up_down):
        ...
        del model
        del current_index  # Otherwise we cannot release the database.
        self.edit_dlg.before_apply_hook = self.unload_qt_database
    ...
    def menu_change_card_type(self):
        ...
        self.controller().change_card_type(
            facts, current_card_type, new_card_type, self.correspondence
        )
        self.card_type_tree_wdgt.rebuild()
        self.tag_tree_wdgt.rebuild()
        self.load_qt_database()
    ...
    def unload_qt_database(self):
        ...
        self.config()[
            "browse_cards_dlg_table_settings"
    ...
    def display_card_table(self, run_filter=True):
        ...
        for key, value in headers.items():
            self.card_model.setHeaderData(
                key, QtCore.Qt.Orientation.Horizontal, QtCore.QVariant(value)
            )
        ...
        self.table.setItemDelegateForColumn(
            QUESTION,
            QA_Delegate(
                QUESTION, component_manager=self.component_manager, parent=self
            ),
        )
        self.table.setItemDelegateForColumn(
            ANSWER,
