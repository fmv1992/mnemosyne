...
class LogDatabase(object):
    ...
    def parse_directory(self):
        ...
        filenames = [
            ...
            for filename in sorted(os.listdir(str(self.log_dir)))
        ...
        for counter, filename in enumerate(filenames):
            ...
            if (
                ...
                print(
                    "(%d/%d) %1.1f%% %s already parsed"
                    % (
                        counter + 1,
                        filenames_count,
                        (counter + 1.0) / filenames_count * 100,
                        os.path.basename(filename),
                    )
                )
                continue
            ...
            self.con.execute(
                "insert into parsed_logs(log_name) values(?)",
                (os.path.basename(filename),),
            )
        self.con.commit()
