...
class TestConvertCards(MnemosyneTest):
    ...
    def setup_method(self):
        ...
        self.mnemosyne.components.append(("test_convert_cards", "Widget"))
    ...
    def test_1_to_2(self):
        ...
        new_fact_data = {"f": "question2", "b": "answer2"}
    ...
    def test_1_to_2_multi(self):
        ...
        self.controller().change_card_type(
            [fact], card.card_type, new_card_type, correspondence=[]
        )
    ...
    def test_3_to_1_c(self):
        ...
        fact_data = {"f": "foreign word", "m_1": "meaning"}
        card_type = self.card_type_with_id("3")
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=-1, tag_names=["default"]
        )[0]
        ...
        new_card_type = self.card_type_with_id("1")
        self.controller().change_card_type(
            [fact], card_type, new_card_type, correspondence=correspondence
        )
    ...
    def test_2_to_2_clone(self):
        ...
        new_card_type = self.controller().clone_card_type(card_type, "my_2")
    ...
    def test_1_to_3_clone(self):
        ...
        fact_data = {"f": "translation", "b": "foreign"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=2, tag_names=["default"]
        )[0]

        new_card_type = self.controller().clone_card_type(
            self.card_type_with_id("3"), "my_language"
        )
        correspondence = {"f": "m_1", "b": "f"}
        self.controller().change_card_type(
        ...
        assert (
            self.database()
            .con.execute("select tags from cards where _id=?", (card_1._id,))
            .fetchone()[0]
            == "default"
        )
        assert (
            self.database()
            .con.execute("select tags from cards where _id=?", (card_2._id,))
            .fetchone()[0]
            == "default"
        )
    ...
    def test_convert_duplicates(self):
        ...
        fact_data = {"loc": "test", "blank": "test1", "marked": "test2"}
        card = self.controller().create_new_cards(
            fact_data, card_type_map, grade=2, tag_names=["default"]
        )[0]

        card_type = self.card_type_with_id("1")
        fact_data = {"f": "test1", "b": "test2"}
        card = self.controller().create_new_cards(
            fact_data, card_type, grade=2, tag_names=["default"]
        )[0]
        ...
        new_fact_data = {"loc": "test", "blank": "test1", "marked": "test2"}
