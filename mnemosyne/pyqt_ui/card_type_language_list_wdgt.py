...
class ComboDelegate(QtWidgets.QItemDelegate, Component):
    ...
    def paint(self, painter, option, index):
        ...
        style.drawComplexControl(
            QtWidgets.QStyle.ComplexControl.CC_ComboBox, opt, painter
        )
    ...
    def data(self, index, role):
        if role == QtCore.Qt.ItemDataRole.DisplayRole:
            ...
            if column == 0:
                ...
                return "" if not key else card_type.name_for_fact_key(key)
    ...
    def setData(self, index, value):
        ...
        if column == 1:
            self.config().set_card_type_property(
                "language_id", self.language_id_with_name[value], card_type
            )
        elif column == 2:
            self.config().set_card_type_property(
                "foreign_fact_key",
...
class CardTypeLanguageListWdgt(QtWidgets.QTableView, Component):
    ...
    def set_card_types(self, card_types):
        self.model = Model(
            component_manager=self.component_manager, card_types=card_types
        )
