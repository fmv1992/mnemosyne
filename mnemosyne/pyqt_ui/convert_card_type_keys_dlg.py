...
class ConvertCardTypeKeysDlg(QtWidgets.QDialog, Ui_ConvertCardTypeKeysDlg):
    ...
    def __init__(
        ...
        **kwds,
        ...
        for (
            old_fact_key,
            old_fact_key_name,
        ) in old_card_type.fact_keys_and_names:
            ...
            for (
                new_fact_key,
                new_key_name,
            ) in new_card_type.fact_keys_and_names:
    ...
    def combobox_updated(self):
        ...
        for (
            old_fact_key,
            old_fact_key_name,
        ) in self.old_card_type.fact_keys_and_names:
            new_fact_key_name = self.comboboxes[old_fact_key].currentText()
            if new_fact_key_name != _("<none>"):
                self.ok_button.setEnabled(True)
                new_fact_key = self.new_card_type.fact_key_with_name(
                    new_fact_key_name
                )
        ...
        if self.check_required_fact_keys:
            for fact_key in self.new_card_type.required_fact_keys:
                if fact_key not in self.correspondence.values():
                    self.ok_button.setEnabled(False)
                    if len(self.correspondence) == len(
                        self.old_card_type.fact_keys()
                    ):
                        QtWidgets.QMessageBox.critical(
                            self,
                            _("Mnemosyne"),
