...
class Mnemosyne(Component):
    ...
    def __init__(
        ...
        try:  # On Android, importlib.util doesn't get found, even though it's present...
            if (importlib.util.find_spec("gtts")) is not None:
                ...
                warnings.warn(
                    "gTTS package is not installed. Text to speak feature will not be available."
                )
            if (importlib.util.find_spec("google_trans_new")) is not None:
                ...
                warnings.warn(
                    "google_trans_new package is not installed. Google translate feature will not be available."
                )
        except:
            pass
    ...
    def register_components(self):
        ...
        for component_name in self.gui_for_component:
            for gui_module_name, gui_class_name in self.gui_for_component[
                component_name
            ]:
                gui_class = getattr(
    ...
    def start_review(self):
        self.controller().set_study_mode(
            self.study_mode_with_id(self.config()["study_mode"])
        )
