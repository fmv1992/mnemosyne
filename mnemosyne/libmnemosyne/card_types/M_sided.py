...
class MSided(CardType):
    ...
    def __init__(self, component_manager):
        ...
        path = os.path.join(
            os.path.dirname(sys.modules["mnemosyne"].__file__),
            "libmnemosyne",
            "renderers",
        )
    ...
    def render_answer(self, card, render_chain="default", **render_args):
        ...
        if (
            ...
            or "<hr id=answer>" not in card.fact_view.extra_data["afmt"]
        ):
            if "FrontSide" in card.fact_view.extra_data["afmt"]:
                new_render_args = copy.copy(render_args)
                new_render_args["body_only"] = True
                render_args["FrontSide"] = self.render_question(
                    card, render_chain, **new_render_args
                )
