...
class ActivateCardsDlg(
    ...
    def update_saved_sets_pane(self):
        ...
        if active_name:
            item = self.saved_sets.findItems(
                active_name, QtCore.Qt.MatchFlag.MatchExactly
            )[0]
    ...
    def save_set(self):
        ...
        if criterion.name in self.criteria_by_name.keys():
            answer = self.main_widget().show_question(
                _("Update this set?"), _("&OK"), _("&Cancel"), ""
            )
            if answer == 1:  # Cancel.
                return
        ...
        self.update_saved_sets_pane()
        item = self.saved_sets.findItems(
            criterion.name, QtCore.Qt.MatchFlag.MatchExactly
        )[0]
        self.saved_sets.setCurrentItem(item)
    ...
    def delete_set(self):
        if not self.saved_sets.currentItem():
            return
        answer = self.main_widget().show_question(
            _("Delete this set?"), _("&OK"), _("&Cancel"), ""
        )
        if answer == 1:  # Cancel.
            return -1
        else:
    ...
    def load_set(self, item, dummy=None):
        ...
        criterion = self.criteria_by_name[name]
        self.tab_widget.setCurrentWidget(
            self.widget_for_criterion_type[criterion.criterion_type]
        )
    ...
    def accept(self):
        ...
        if (
            ...
            and self.config()["showed_help_on_double_clicking_sets"] == False
        ):
            self.main_widget().show_information(
                _(
                    "You can double-click on the name of a saved set to activate it and close the dialog."
                )
            )
