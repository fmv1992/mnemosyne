...
def startup():
    ...
    mnemosyne.initialise(
        data_dir=os.path.abspath("dot_benchmark"), automatic_upgrades=False
    )
...
def grade_only():
    mnemosyne.scheduler().grade_answer(mnemosyne.review_controller().card, 0)
...
def activate():
    ...
    c._tag_ids_active = set(
        [mnemosyne.database().get_or_create_tag_with_name("default")._id]
    )
...
def do_import():
    mnemosyne.component_manager.current("file_format").do_import(
        "/home/pbienst/dot_mnemosyne/default.mem"
    )
...
def test_setup():
    ...
    mnemosyne.components.append(("test_add_cards", "Widget"))
    mnemosyne.gui_for_component["ScheduledForgottenNew"] = [
        ("mnemosyne_test", "TestReviewWidget")
    ]
    mnemosyne.components.append(
