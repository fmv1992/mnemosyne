...
class TagDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def setEditorData(self, editor, index):
        ...
        self.previous_node_name = (
            index.model().data(node_index).replace("::" + _("Untagged"), "")
        )
...
class TagsTreeWdgt(Component, QtWidgets.QWidget):
    ...
    def __init__(self, acquire_database=None, **kwds):
        ...
        self.tree_wdgt.setContextMenuPolicy(
            QtCore.Qt.ContextMenuPolicy.CustomContextMenu
        )
        self.tree_wdgt.customContextMenuRequested.connect(self.context_menu)
    ...
    def create_tree(self, tree, qt_parent):
        for node in tree:
            ...
            node_item.setData(
                NODE, QtCore.Qt.ItemDataRole.DisplayRole, QtCore.QVariant(node)
            )
    ...
    def restore_criterion(self):
        ...
        for tag in self.database().tags():
            if (
                tag._id in self.saved_criterion._tag_ids_active
                or tag._id not in self.saved_criterion._tag_ids_forbidden
            ):
